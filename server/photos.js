...
router.post("/photos", verifyToken, async (req, res) => {
  ...
  if (!unsplash_id || !photographer) {
    return res
      .status(400)
      .json({ error: "Unsplash ID and Photographer are required " });
  }

  try {
    // Checking if photo already exists in database
    const existingPhoto = await client.query(
      "SELECT id FROM photos WHERE unsplash_id = $1",
      [unsplash_id]
    );

    if (existingPhoto.rows.length > 0) {
      console.log(
        "Photo already exists, returning existing ID:",
        existingPhoto.rows[0].id
      );
    ...
    const result = await client.query(
      "INSERT INTO photos (unsplash_id, photographer) VALUES ($1, $2) RETURNING id",
      [unsplash_id, photographer]
    );
