...
router.get("/entries", verifyToken, async (req, res) => {
  ...
  try {
    const result = await client.query(
      "SELECT id, photo_id, content, created_at FROM journal_entries WHERE user_id = $1 ORDER BY created_at DESC",
      [userId]
    );
...
router.get("/entries/:entryId?", async (req, res) => {
  ...
  try {
    ...
    const photo = await client.query("SELECT * FROM photos WHERE id = $1", [
      journalEntry.photo_id,
    ]);
